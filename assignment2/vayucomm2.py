...
def client_sender(client_socket,data_dict):
    while True:
        ...
        # lines.append(line)
        data = recv_input(client_socket)
        print('Server response:', data)

def client_vayu(client_socket,client_socket_2,data_dict,start):
...
def main():
    ...
    while True:
        ...
        if choice == 'c1':
            ...
            client_thread = threading.Thread(target=client_vayu, args=(client_socket,client_socket_2,data_dict,start))
            client_thread.start()
            client_thread.join()
        elif choice == 's':
